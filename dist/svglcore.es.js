...
class Renderer {
  ...
  */
  ...
  _addBoundingBox(mesh, mvpMat, bbLineThickness = 0.33) {
    ...
    vec3.transformMat4(tmpVector, a3D, mvpMat);
    ...
    vec3.transformMat4(tmpVector, b3D, mvpMat);
    ...
    vec3.transformMat4(tmpVector, c3D, mvpMat);
    ...
    vec3.transformMat4(tmpVector, d3D, mvpMat);
    ...
    vec3.transformMat4(tmpVector, e3D, mvpMat);
    ...
    vec3.transformMat4(tmpVector, f3D, mvpMat);
    ...
    vec3.transformMat4(tmpVector, g3D, mvpMat);
    ...
    vec3.transformMat4(tmpVector, h3D, mvpMat);
  ...
  _renderFaceOpaquePlain(mesh, mvpMat) {
    ...
    for (let f = 0; f < nbFaces; f += 1) {
      ...
      vec3.transformMat4(tmp, normalTip, mvpMat);
...
class PointLight extends Light {
  ...
  computeLight(options = {}) {
    ...
    if (specularity > 0) {
      ...
      const dotLightToNormal = vec3.dot(illuminatedNormal, tmpVec3);
      ...
      if (dotLightToNormal < 0) {
        ...
        // 6. normalize this
        vec3.normalize(tmpVec3_2, tmpVec3_2);
        // 7. compute the dot product to have the specularity component
        const dotProd2 = vec3.dot(tmpVec3, tmpVec3_2) ** (30 / specularity); // the 2 is just to make the light smaller and more intense
